...
- **PDF Word Repair**: Post-extraction `repair_broken_words()` function fixes mid-word spaces introduced by PyPDF2 (e.g., "gover nance" → "governance"). Uses a curated domain word set + self-referencing heuristic + suffix-based word recognition. Also handles Unicode ligature artifacts (fi, fl, ff). Enhanced to handle multi-fragment breaks (3+ pieces), single-letter fragment breaks (e.g., "a ccountability"), and breaks at morpheme boundaries.
- **Spelling Validation Layer**: Programmatic spell-check validation (pyspellchecker) runs after LLM analysis to discard false spelling errors. Validates that flagged words are actually misspelled before showing them. Handles: valid words incorrectly flagged by LLM, PDF-broken words where joined text is valid, both-valid-word pairs (silently discarded unless American-vs-British). Test suite in `test_spelling_validation.py`.
